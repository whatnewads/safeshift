-- SafeShift EHR Complete Database Schema (FIXED)
...
-- Add missing columns to audit_log if needed
ALTER TABLE audit_log ADD COLUMN IF NOT EXISTS checksum CHAR(64);
...
CREATE TABLE IF NOT EXISTS staff_training_records (
    ...
    status VARCHAR(20) AS (
        CASE
            ...
            WHEN expiration_date <= DATE_ADD(CURRENT_DATE, INTERVAL 30 DAY) THEN 'expiring_soon'
            ELSE 'current'
        END
    ) STORED,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
-- Insert default flag rules (if not exists)
INSERT IGNORE INTO flag_rules (rule_id, rule_name, rule_type, rule_condition, flag_severity, is_active) VALUES
(UUID(), 'High Blood Pressure', 'vital_threshold', '{"field": "systolic_bp", "operator": ">", "value": 180}', 'critical', TRUE),
(UUID(), 'Very High Blood Pressure', 'vital_threshold', '{"field": "diastolic_bp", "operator": ">", "value": 110}', 'critical', TRUE),
(UUID(), 'Missing Vitals', 'missing_field', '{"field": "vitals", "required": true}', 'high', TRUE),
(UUID(), 'Incomplete Documentation', 'missing_field', '{"field": "chief_complaint", "required": true}', 'medium', TRUE),
(UUID(), 'OSHA Recordable', 'injury_severity', '{"field": "injury_type", "operator": "in", "value": ["lost_time", "hospitalization"]}', 'high', TRUE);

-- Insert training requirements (if not exists)
INSERT IGNORE INTO training_requirements (requirement_id, training_name, training_description, training_category, required_roles, recurrence_interval) VALUES
(UUID(), 'HIPAA Privacy Training', 'Annual HIPAA privacy and security training', 'compliance', '["1clinician", "pclinician", "dclinician", "cadmin", "tadmin"]', 365),
(UUID(), 'OSHA Safety Training', 'Occupational safety and health training', 'safety', '["1clinician", "pclinician", "dclinician"]', 365),
(UUID(), 'BLS Certification', 'Basic Life Support certification', 'clinical', '["1clinician", "pclinician", "dclinician"]', 730),
(UUID(), 'DOT Examiner Training', 'DOT Medical Examiner certification', 'clinical', '["pclinician"]', 1825);

-- Insert compliance KPIs (if not exists)
INSERT IGNORE INTO compliance_kpis (kpi_id, kpi_name, kpi_category, calculation_method, threshold_warning, threshold_critical, unit) VALUES
(UUID(), 'Chart Completion Rate', 'clinical', 'SELECT (COUNT(CASE WHEN status = "complete" THEN 1 END) * 100.0) / COUNT(*) FROM encounters WHERE created_at >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)', 90.00, 80.00, 'percentage'),
(UUID(), 'OSHA TRIR', 'osha', 'SELECT (COUNT(*) * 200000.0) / (SELECT SUM(hours_worked) FROM employee_hours WHERE year = YEAR(CURDATE())) FROM osha_cases WHERE recordable = TRUE AND year = YEAR(CURDATE())', 3.00, 4.00, 'rate'),
(UUID(), 'DOT Test Completion', 'dot', 'SELECT (COUNT(CASE WHEN status = "completed" THEN 1 END) * 100.0) / COUNT(*) FROM dot_tests WHERE test_type = "random" AND year = YEAR(CURDATE())', 95.00, 90.00, 'percentage'),
(UUID(), 'Training Compliance', 'hipaa', 'SELECT (COUNT(CASE WHEN status = "current" THEN 1 END) * 100.0) / COUNT(*) FROM staff_training_records WHERE requirement_id IN (SELECT requirement_id FROM training_requirements WHERE training_category = "compliance")', 98.00, 95.00, 'percentage');

-- Insert tooltips (if not exists)
INSERT IGNORE INTO ui_tooltips (tooltip_id, field_identifier, tooltip_text, tooltip_type, role_filter) VALUES
(UUID(), 'vitals.systolic_bp', 'Enter systolic blood pressure (top number). Normal range: 90-120 mmHg. Flag if >180 mmHg.', 'info', 'all'),
(UUID(), 'vitals.diastolic_bp', 'Enter diastolic blood pressure (bottom number). Normal range: 60-80 mmHg. Flag if >110 mmHg.', 'info', 'all'),
(UUID(), 'chief_complaint', 'Document the patient''s main reason for visit in their own words. This field is required for all encounters.', 'help', 'all'),
(UUID(), 'dot_test.specimen_id', 'Enter the Chain of Custody form number. Must match the specimen label exactly.', 'compliance', '1clinician,pclinician'),
(UUID(), 'injury.osha_recordable', 'Check if this injury results in: death, days away from work, restricted work/transfer, medical treatment beyond first aid, loss of consciousness, or significant diagnosed injury/illness.', 'warning', 'all');